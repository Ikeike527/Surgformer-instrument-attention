...
from datasets.phase.Cholec80_phase import CHOLEC80_SPLIT_TAG
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="Parameters",
        help="Optional initialization checkpoint path",
    )
    ...
    parser.add_argument(
        "--disable_spatial_black_mask",
        action="store_true",
        help="Disable masking fully-black spatial patches before attention softmax.",
    )
    parser.add_argument(
        "--black_pixel_threshold",
        type=float,
        default=15.0 / 255.0,
        help="Pixel threshold in [0,1] used to detect black border regions.",
    )
    parser.add_argument(
        "--ablate_offfield",
        action="store_true",
        help="Zero (to true black) input pixels outside the circular FOV before the model (Step3 causal test).",
    )
    parser.add_argument(
        "--offfield_radius_scale",
        type=float,
        default=1.0,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="器具マスク強調の強さ。softmax 前に lambda*被覆率 を key 列へ加算。0 で無効。",
    )
    parser.add_argument(
        "--instr_bias_blocks",
        default="all",
        help='attn_bias を適用する Block。"all" または整数N (最終N Block のみ)。',
    )
    parser.add_argument(
        "--instr_mask_dirname",
        default="instrument_masks",
        help="data_path 下のマスクディレクトリ名。",
    )
    parser.add_argument(
        "--drop_path",
        type=float,
    ...
    parser.add_argument(
        "--output_dir",
        default="outputs/AutoLaparo",
        help="path where to save, empty for no saving",
    )
    parser.add_argument(
        "--split_tag",
        default="",
        help="optional dataset split tag appended to the run directory",
    )
    parser.add_argument(
        "--log_dir",
        default="outputs/AutoLaparo",
...
def resolve_best_checkpoint_path(output_dir):
    candidates = [
        Path(output_dir) / "checkpoint-best.pth",
        Path(output_dir) / "checkpoint-best" / "mp_rank_00_model_states.pt",
    ]
    ...
    raise FileNotFoundError(
        "Could not find a best checkpoint under %s. Expected one of: %s"
        % (output_dir, ", ".join(str(candidate) for candidate in candidates))
    )


def resolve_split_tag(args):
    if args.split_tag:
        return args.split_tag
    if args.data_set == "Cholec80":
        return CHOLEC80_SPLIT_TAG
    return ""
...
def main(args, ds_init):
    ...
    args.split_tag = resolve_split_tag(args)
    output_name_parts = [
        args.model,
        args.data_set,
    ]
    if args.split_tag:
        output_name_parts.append(args.split_tag)
    ...
    preds_file = os.path.join(args.output_dir, str(global_rank) + ".txt")
    best_pretrained_path = resolve_best_checkpoint_path(args.output_dir)
