...
GPUS="${GPUS:-1}"
MASTER_PORT="${MASTER_PORT:-12326}"
MODEL="${MODEL:-surgformer_HTA}"
DATA_SET="${DATA_SET:-Cholec80}"
DATA_PATH="${DATA_PATH:-/workspace/data/Cholec80}"
...
if [[ -z "${FINETUNE_PATH}" ]]; then
  echo "FINETUNE_PATH must point to a trained checkpoint." >&2
  exit 1
fi
...
if [[ "${ABLATE_OFFFIELD:-0}" == "1" ]]; then
  EXTRA_ARGS+=(--ablate_offfield)
fi
