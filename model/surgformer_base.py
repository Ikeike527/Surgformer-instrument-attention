...
class Attention_Spatial(nn.Module):
    ...
    def forward(self, x, B, patch_mask=None):
        BT, K, C = x.shape
        T = BT // B
        qkv = self.qkv(x)
...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, K, patch_mask=None):
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            x = blk(x, B, T, K, patch_mask=spatial_patch_mask)
...
if __name__ == "__main__":
    model = surgformer_base(pretrained=False)
